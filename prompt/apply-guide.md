...
| 02-behavior-style.md | `[personality]` → `behavior_style` | 整段粘贴（已含机制联动精简版 + 工具采用段） |
...
| 06-mechanism.md | 无（完整参考版） | 机制细则需要扩写时查阅 |
...
2. **机制规则已并入 02**：`behavior_style` 只进 Planner。02 的成品文本已含机制联动精简版（等级半透明/日程/备忘/睡眠/系统边界等）与工具采用段（点名 memo_write/update_schedule/msg_react/poke_user/inspect_image，鼓励 Planner 主动使用）；如需更细的机制行为细则，从 06 对应章节扩写。若担心 Replyer 生成时不受系统边界约束，Replyer 侧由 03 的"不说系统词汇"性质底线 + 04 的硬底线兜底。
